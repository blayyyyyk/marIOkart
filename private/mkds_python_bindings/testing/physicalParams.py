from ctypes import *
from private.mkds_python_bindings.testing.fx import *
from private.mkds_python_bindings.testing.types import *


class physp_kart_params_t(Structure):
    _fields_ = [
        ('colSphereSize', fx32),
        ('colSphereZOffset', fx32),
        ('gap8', (u8 * 4)),
        ('weight', fx16),
        ('driftBoostTime', s16),
        ('maxSpeed', fx32),
        ('baseAcceleration', fx32),
        ('field18', fx32),
        ('field1C', fx32),
        ('driftBaseAcceleration', fx32),
        ('field24', fx32),
        ('field28', fx32),
        ('deceleration', fx32),
        ('handling', fx16),
        ('drift', fx16),
        ('driftTurningCompensation', fx32),
        ('collisionVelocityMinusDirMultipliers', (fx32 * 12)),
        ('collisionSpeedMultipliers', (fx32 * 12)),
    ]

class physp_char_params_t(Structure):
    _fields_ = [
        ('field0', fx32),
        ('weight', fx32),
    ]

class physp_t(Structure):
    _fields_ = [
        ('driverKartPhysicalParams', u32), #POINTER(physp_kart_params_t)),
        ('driverCharPhysicalParams', u32), #POINTER(physp_char_params_t)),
        ('field8', u32),
        ('kartPhysicalParams', u32), #POINTER(physp_kart_params_t)),
        ('charPhysicalParams', u32), #POINTER(physp_char_params_t)),
    ]
