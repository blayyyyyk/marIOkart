...
INTERIM_DATASET_PATH = DATASET_PATH / "interim"  # this is where collected observation data is held
...
BATCH_SIZE = 128  # number of training/testing examples processed by the model in parallel
...
NUM_FEATURES = RAY_COUNT + EMBED_SIZE  # number of dims of the feature vector fed into the model. part of this will always be the keymask embed
TOTAL_TRAINING_TIMESTEPS = 20000  # number of training epochs for a training session
SPLIT_RATIO = 0.8  # ratio of training examples to total dataset examples provided for training
LEARNING_RATE = 1e-4  # this will likely need to be tweaked if not using the Adam optimizer
