from gymnasium.core import ActType, ObsType, ActType, WrapperObsType
from gym_mkds.wrappers.sweeping_ray import find_current_boundary_lines, project_to_plane, get_standing_triangle_id
...
import numpy as np
from typing import TypeVar, Callable, Concatenate, cast, Literal
...
def closest_track_point(emu: MarioKart, direction_mode: Literal["movement"] | Literal["direction"] = "movement", default_collision_type: int = 0, eps: float = 1e-7) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    p_intersect = project_2d(boundary_lines[:, 0], boundary_lines[:, 1], kart_position)
