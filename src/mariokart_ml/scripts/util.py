...
def check_device_cpu(args):
    ...
    if args.device == "cpu":
        warn("CPU device detected, training model performance may be impacted. Consider selecting another device with the device flag.", stacklevel=2)
...
for env_id, spec in gym.envs.registry.items():
    if not isinstance(spec.entry_point, str):
        continue
    if spec.entry_point.startswith("gymnasium.envs"):
        continue
