...
class MarioKartDataset(Dataset):
    def __init__(self, folder_path: str, window: int):
        ...
        self.mdata = {}
        self.window = window
        with open(self.folder_path / "mdata.json") as f:
        ...
        for key, value in self.mdata.items():
            ...
            self.data[key] = np.memmap(str(path), dtype=value["dtype"], mode="r", shape=tuple(value["shape"]))
            self.batch_size = value["shape"][0]
            if not ("std" in self.mdata[key] and "mean" in self.mdata[key]):
                continue
...
class ConcatMarioKartDataset(ConcatDataset):
    ...
    def compute_global_stats(self) -> dict:
        ...
        keys_to_normalize = [k for k, v in first_mdata.items() if "mean" in v and "std" in v]
        ...
        for key in keys_to_normalize:
            ...
            global_stats[key] = {"mean": global_mean, "std": global_std}
