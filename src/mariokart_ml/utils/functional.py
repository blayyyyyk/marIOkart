...
def _async_env_worker(func: Callable, create_env_fn: Callable, vec_class: Callable, *list_args):
    env_fns = [(lambda k=k: create_env_fn(*k)) for k in zip(*list_args, strict=True)]
...
def sub_process_func(func: Callable, create_env_fn: Callable, vec_class: Callable = AsyncVectorEnv):
    def _wrapper(*args):
        ...
        p = multiprocessing.Process(target=_async_env_worker, args=(func, create_env_fn, vec_class, *args))
        p.start()
