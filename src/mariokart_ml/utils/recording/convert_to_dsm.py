...
def do_menu(emu, character: int, kart: int, course: int):
    ...
    for _i in range(character_x):
        press(Keys.KEY_RIGHT)
        wait(7)
    for _i in range(character_y):
        press(Keys.KEY_DOWN)
        wait(7)
    press(Keys.KEY_A)
    wait(30)
    ...
    if kart % 3 == 0:
        ...
        remaining_rights = 2
    for _i in range(base_kart_order_placement + remaining_rights):
        press(Keys.KEY_RIGHT)
        wait(7)
    press(Keys.KEY_A)
    ...
    cup_x = cup % 4
    cup_y = cup // 4
    for _i in range(cup_x):
        press(Keys.KEY_RIGHT)
        wait(1)
    for _i in range(cup_y):
        press(Keys.KEY_DOWN)
        wait(1)
    press(Keys.KEY_A)
    wait(55)
    for _i in range(ordered_course_number % 4):
        press(Keys.KEY_DOWN)
        wait(1)
    press(Keys.KEY_A)
    wait(15)
...
def run(input_file: str, output_file: str | None = None):
    ...
    convert_to_dsm(output_file, **data)
