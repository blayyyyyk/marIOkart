...
def encrypt_in_place(data: bytearray, m4: int) -> bool:
    ...
    for i in range(0, len(data), 4):
        ...
        m1 += 1
        if m1 < len(mask1): continue
        m1 = 0
        m2 += 1
        if m2 < len(mask2): continue
        m2 = 0
        m3 += 1
        if m3 < len(mask3): continue
        m3 = 0
        m4 = (m4 + 1) % len(mask4)
...
def has_ghost(data: bytearray, ordered_course_id: int) -> bool:
    if ordered_course_id < 0 or ordered_course_id > 31:
        raise Exception("Invalid course ID given. Expected 0-31.")
...
def main(save_path, course, out_path):
    ...
    if course == 'all':
        ...
        if has_ghost(contents, course):
            print("no data found for specified course id")
...
if __name__ == '__main__':
    ...
    course = sys.argv[2]
    out_path = sys.argv[3]
    main(save_path, course, out_path)
