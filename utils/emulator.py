...
class Triangle(DrawObject):
    def __init__(
        self, p1: Point, p2: Point, p3: Point, stroke_width: int | None = None
    ):
...
class Scene:
    ...
    def add_lines(
        ...
        for p1, p2 in zip(pts1, pts2):
            line_obj = Line(Point(*p1), Point(*p2), stroke_width=stroke_width)
...
def draw_points(
    scene: Scene, pts: list[list[float]], color: tuple[float, float, float]
):
