...
class Racer:
    def __init__(
        self, kcl: KCL, nkm: NKM, z_near=0.0, z_far=1000.0, z_scale=10.0, device=None
    ):
        self.device = device
        self.kcl = kcl
        self.nkm = nkm
    ...
    @classmethod
    def from_path(
        cls, kcl_path: str, nkm_path: str, z_near=0.0, z_far=100.0, device=None
    ):
    ...
    def position(self) -> torch.Tensor:
        if self.racer_data_addr is None:
            ...
            self.racer_data_addr = self.memory.read_long(
                DEFAULT_RACER_ADDRS["racer_data_ptr"]
            )
    ...
    def camera_details(self) -> tuple[float, float]:
        if self.camera_data_addr is None:
            assert self.memory is not None
            self.camera_data_addr = self.memory.read_long(
                DEFAULT_RACER_ADDRS["camera_data_ptr"]
            )

        fov = read_u16(self.memory, self.camera_data_addr + 0x60) * (
            2 * math.pi / 0x10000
        )
    ...
    def model_view(self) -> torch.Tensor:
        return compute_model_view(
            self.camera_position, self.camera_target_position, device=self.device
        )

    def get_checkpoint_details(self):
        assert self.memory is not None
        if self.checkpoint_data_addr is None:
            self.checkpoint_data_addr = read_u32(
                self.memory, DEFAULT_RACER_ADDRS["checkpoint_data_ptr"]
            )
    ...
    def get_facing_point_checkpoint(
        self, position: torch.Tensor, direction: torch.Tensor
    ):
        ...
        intersect = torch.tensor(
            [intersect[0], position[1], intersect[1]], device=self.device
        )
        return intersect
    ...
    def get_facing_point_obstacle(
        self, position: torch.Tensor, direction: torch.Tensor
    ):
        triangles = self.kcl.triangles
        wall_mask = self.kcl.prisms.is_wall == 1
        offroad_mask = (
            (self.kcl.prisms.collision_type == 5)
            | (self.kcl.prisms.collision_type == 3)
            | (self.kcl.prisms.collision_type == 2)
        )
